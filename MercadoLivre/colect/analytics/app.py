...
# criando conexao
conn = create_engine('postgresql://postgres:bloqeiodofgh@localhost:5432/mercadolivre')

# Carregar os dados da tabela 'mercadolivre_items' em um DataFrame pandas
df = pd.read_sql_query("SELECT * FROM dadosml", conn)

conn.dispose() # Fechar conexão
...
# KPI 4: Número total de reviews
total_reviews = df['reviews_amount'].sum()
col4.metric(label="Número Total de Reviews", value=total_reviews)
...
# Review por marca
st.subheader('Review por marca')
col1, col2 = st.columns([4, 2])
df_non_zero_reviews = df[df['reviews_rating_number'] > 0]
review_by_brand = df_non_zero_reviews.groupby('brand')['reviews_rating_number'].count().sort_values(ascending=False)
col1.bar_chart(review_by_brand)
col2.write(review_by_brand)